...
| `saltSyntax.preview.pythonPath` | `python3` | *(Experimental preview)* Python interpreter for the rendered preview — needs `jinja2` and `pyyaml`. |
| `saltSyntax.preview.fileRoots` | `[]` | *(Experimental preview)* Salt file roots imports, `include:` targets and `sls`/`tpldir` resolve against. Empty: the workspace folder, or with no folder open, the directory above the file's own. |
